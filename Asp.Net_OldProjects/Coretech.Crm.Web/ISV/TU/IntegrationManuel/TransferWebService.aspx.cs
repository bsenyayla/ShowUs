...
using UPT.WebServices.Extension.TransferWService;

public partial class IntegrationManuel_TransferWebService : Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            if (CheckUserOnline())
            {
                ...
                MethodInfo[] methods = typeof(TransferWebService).GetMethods();
    ...
    {
        TransferWebService service = new TransferWebService();
        service.GetType().GetProperty("ByPassLogin", System.Reflection.BindingFlags.GetProperty | System.Reflection.BindingFlags.NonPublic | BindingFlags.Instance).SetValue(service, true, null);
        
        DynamicFactory df = new DynamicFactory(ERunInUser.CalingUser);
        DynamicEntity de = df.RetrieveWithOutPlugin(1, App.Params.CurrentUser.SystemUserId, new string[] { "UserName" });
        string tempUserName = de.GetStringValue("UserName");

        service.UserInfo = new Coretech.Crm.Provider.Security.WsSystemUserInfo() { Username = tUserName.Text };
        ...
        service.UserInfo = new Coretech.Crm.Provider.Security.WsSystemUserInfo() { Username = tempUserName };
        service.UserLoginWithoutPassword();
