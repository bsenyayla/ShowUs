...
public partial class CustAccount_Pool_DepositTransaction : BasePage
{
    ...
    private void SetDefaults()
    {
        ...
        lblnew_SenderType3.FieldLabel =
        mfSenderType.FieldLabel =
            CrmLabel.TranslateAttributeLabelMessage("new_SenderType", TuEntityEnum.New_CustAccountOperations.GetHashCode());
...
LEFT OUTER JOIN vnew_custaccountOperationType cao on cao.new_EXTCODE='002' and cao.DeletionStateCode=0
...
")
    ...
    protected void btn3rdSenderEditUpdate_Click(object sender, AjaxEventArgs e)
    {
        if (string.IsNullOrEmpty(new_CustAccountTypeId.Value))
        {
            ShowRequiredFields(new_CustAccountTypeId.FieldLabel);
        ...
        {
            {"ObjectId", ((int) TuEntityEnum.New_Sender).ToString()},
            {"fromCustomerAccount3rdPerson", "1"},
            {"gridpanelid", ""},
            {"defaulteditpageid","5d08acf8-39ad-e511-9f11-28e347b36ba3"}
        };

        if (new_3rdSenderId.Value != string.Empty)
        {
            query.Add("recid", new_3rdSenderId.Value);
        }
        var urlparam = QueryHelper.RefreshUrl(query);
        QScript("window.top.newWindowRefleX(window.top.GetWebAppRoot + '/CrmPages/AutoPages/EditReflex.aspx" + urlparam + "', { maximized: false, width: 1100, height: 500, resizable: true, modal: true, maximizable: false });");
    ...
    protected void new_CustAccountId_OnChange(object sender, AjaxEventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(new_CustAccountId.Value))
        {
            ...
            var isRestricted = new CustAccountOperations().IsRestrictedCustAccount(Object.TransactionTypeEnum.Deposit, custAccountId);
            if (isRestricted)
            {
                new_CustAccountId.Clear();
                Upt.AlertMessage(
                    string.Format(CrmLabel.TranslateMessage("CRM.NEW_CUSTACCOUNTOPERATIONS_RESTRICTEDFORDEPOSIT"), new_CustAccountId.SelectedItems[0].VALUE)
                    );
    ...
    protected void new_3rdSenderId_OnChange(object sender, AjaxEventArgs e)
    {
        Refresh3rdSender(ValidationHelper.GetGuid(new_3rdSenderId.Value));
    }
    private void Refresh3rdSender(Guid senderId)
    {
        var ms = new MessageBox { Modal = true };
        ...
        if (senderId != Guid.Empty)
        {
            ...
            var urlparam = QueryHelper.RefreshUrl(query);
            Sender3rdDetail.AutoLoad.Url =
                Page.ResolveClientUrl("~/CrmPages/AutoPages/EditReflex.aspx" + urlparam);
            Sender3rdDetail.LoadUrl(Sender3rdDetail.AutoLoad.Url);
            QScript(" R.reSize();");
        }
        else
        {
    ...
    private void CustAccountCurrencyIdChanged()
    {
    ...
    protected void new_SenderTypeOnChange(object sender, AjaxEventArgs e)
    {
        isNewAmount = true;
    ...
    protected void CalculateOnEvent(object sender, AjaxEventArgs e)
    {
        ...
        try
        {
            ...
            sd.AddParameter("SenderType", DbType.Int16, ValidationHelper.GetInteger(new_SenderType.Value));
    ...
    private void ClearFieldsForCustAccountID()
    {
        ...
        new_ReceivedAmount1.c1.Clear();
        new_ReceivedAmount1.d1.Clear();
        new_ReceivedAmount2.c1.Clear();
        new_ReceivedAmount2.d1.Clear();
        new_CollectionMethod.Clear();
        new_TotalReceivedAmount.c1.Clear();
        new_TotalReceivedAmount.d1.Clear();
    }
    ...
    protected void RebuildExpenseProcess(Guid custAccountOperationsId, DbTransaction transaction)
    {
        ...
        try
        {
            ...
            //sd.AddParameter("CountryID", DbType.Guid, ValidationHelper.GetDBGuid(new_CorporationCountryId.Value));
            sd.AddParameter("CorporationID", DbType.Guid, ValidationHelper.GetDBGuid(new_CorporationId.Value));
            sd.AddParameter("CustAccountOperationTypeId", DbType.Guid,
                ValidationHelper.GetDBGuid(new_CustAccountOperationTypeId.Value));
            //sd.AddParameter("RecipientCountryID", DbType.Guid, ValidationHelper.GetDBGuid(new_RecipientCountryID.Value));
            sd.AddParameter("Amount", DbType.Decimal, amount);
            sd.AddParameter("AmountCurrency", DbType.Guid, ValidationHelper.GetDBGuid(new_CustAccountCurrencyId.Value));
            sd.AddParameter("ReceivedAmount1Currency", DbType.Guid,
                ValidationHelper.GetDBGuid(((CrmComboComp)new_ReceivedAmount1.Items[1]).Value));
            sd.AddParameter("ReceivedAmount1", DbType.Decimal, ramount);
            //sd.AddParameter("TransactionId", DbType.Guid, DBNull.Value);
            sd.AddParameter("SenderId", DbType.Guid, ValidationHelper.GetDBGuid(new_SenderId.Value));
            sd.AddParameter("CollectionMethod", DbType.Int32, ValidationHelper.GetInteger(new_CollectionMethod.Value));
            ...
            {
                sd.AddParameter("CustomExpense", DbType.Decimal, new_CalculatedExpenseAmount.d1.Value);
                sd.AddParameter("CustomExpenseCurrency", DbType.Guid,
                    ValidationHelper.GetDBGuid(new_CalculatedExpenseAmount.c1.Value));
            }
            ...
            else
            {
                ...
                if (result.Rows.Count > 0)
                {
                    if (result.Columns.Contains("ErrorMessage"))
                    {
                        ...
                        if (result.Columns.Contains("CalculatedExpenseAmount"))
                        {
                            new_ExpenseAmount.Items[0].SetValue(
                                ValidationHelper.GetDecimal(result.Rows[0]["CalculatedExpenseAmount"], 0));
                            new_ReceivedAmount1.d1.SetValue(
                                ValidationHelper.GetDecimal(result.Rows[0]["new_ReceivedAmount1"], 0));
                        }
                    ...
                    else
                    {
                        ...
                        new_ReceivedAmount1.Items[0].SetValue(ValidationHelper.GetDecimal(
                            result.Rows[0]["TAHSILTUTARI"], 0));
                        new_ReceivedAmount1.c1.SetValue(
                            ValidationHelper.GetString(result.Rows[0]["TAHSILTUTARIDOVIZ"]),
                            ValidationHelper.GetString(result.Rows[0]["TAHSILTUTARIDOVIZTEXT"]));

                        new_ReceivedExpenseAmount.Items[0].SetValue(
                            ValidationHelper.GetDecimal(result.Rows[0]["NETMASRAFTUTARI"], 0));
                        new_ReceivedExpenseAmount.Items[1].SetValue(
                            ValidationHelper.GetString(result.Rows[0]["NETMASRAFTUTARIDOVIZ"]));

                        new_ExpenseAmount.Items[0].SetValue(
                            ValidationHelper.GetDecimal(result.Rows[0]["INDIRIMLIMASRAFTUTARI"], 0));
                        new_ExpenseAmount.Items[1].SetValue(
                            ValidationHelper.GetString(result.Rows[0]["INDIRIMLIMASRAFTUTARIDOVIZ"]));

                        if (new_CalculatedExpenseCurrencyDefaultValue.Value !=
                            ValidationHelper.GetString(result.Rows[0]["MASRAFTUTARIDOVIZ"]) ||
                            ...
                            new_CalculatedExpenseAmountDefaultValue.Value == string.Empty)
                        {
                            new_CalculatedExpenseAmountDefaultValue.Value =
                                ValidationHelper.GetDecimal(result.Rows[0]["MASRAFTUTARI"], 0).ToString();
                            new_CalculatedExpenseCurrencyDefaultValue.Value =
                                ValidationHelper.GetString(result.Rows[0]["MASRAFTUTARIDOVIZ"]);
                            new_CalculatedExpenseAmountDefaultValue.SetIValue(
                                ValidationHelper.GetDecimal(result.Rows[0]["MASRAFTUTARI"], 0));
                        ...
                        new_TotalReceivedAmount.Items[0].SetValue(
                            ValidationHelper.GetDecimal(result.Rows[0]["TOPLAMTAHSIL"], 0));
                        new_TotalReceivedAmount.Items[1].SetValue(
                            ValidationHelper.GetString(result.Rows[0]["TOPLAMTAHSILDOVIZ"]));

                        Parity1.Clear();
                        Parity1.SetHtml("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" +
                                        CrmLabel.TranslateAttributeLabelMessage("new_TransferPaymentCurrencyParity1",
                                            TuEntityEnum.New_Transfer.GetHashCode()) + "&nbsp;&nbsp;" +
                                        Math.Round(
                                            ValidationHelper.GetDecimal(
                                                result.Rows[0]["new_TransferPaymentCurrencyParity1"], 0), 6).ToString());
                    }
                }
            }
        }
    ...
    protected void new_CountryIdentificationTypeLoad(object sender, AjaxEventArgs e)
    {
        ...
        var differentSender = new_3rdSenderId.Value;
        if (String.IsNullOrEmpty(differentSender) == false) //3.Şahıs seçilmiş
        {
            trt = dft.RetrieveWithOutPlugin(TuEntityEnum.New_Sender.GetHashCode(), ValidationHelper.GetGuid(differentSender), new string[] { "new_NationalityID" });
        }
        else
        {
            if (!isTuzel)
...
Declare @Data table(new_IdentificationCardType UniqueIdentifier, new_IdentificationCardTypeName nvarchar(100))
    ...
    protected void btnFinishOnClickEvent(object sender, AjaxEventArgs e)
    {
        ...
        #region AgeControl
        CustAccountOperations custOp = new CustAccountOperations();
        var differentSender = new_3rdSenderId.Value;
        if (String.IsNullOrEmpty(differentSender) == false) //3.Şahıs seçilmiş
        {
            bool isAdult = custOp.IsSenderAdult(ValidationHelper.GetGuid(differentSender));
            if (isAdult == false)
            {
        ...
        try
        {
            ...
            de.AddLookupProperty("new_3rdSenderId", "", ValidationHelper.GetGuid(new_3rdSenderId.Value));
            de.AddLookupProperty("new_CustAccountId", "", ValidationHelper.GetGuid(new_CustAccountId.Value));
            de.AddPicklistProperty("new_SenderType", ValidationHelper.GetInteger(new_SenderType.Value,0));
            ...
            sd.AddParameter("SystemUserId", DbType.Guid, App.Params.CurrentUser.SystemUserId);
            var ret = ValidationHelper.GetString(sd.ExecuteScalarSp("SpTuCustAccountDepositCreateCheck", tr));
        ...
        if (id != Guid.Empty)
        {
            ...
            sd.AddParameter("CustAccountOperationsId", DbType.Guid, id);
            sd.AddParameter("SystemUserId", DbType.Guid, App.Params.CurrentUser.SystemUserId);
            var ret1 = ValidationHelper.GetString(sd.ExecuteScalarSp("SpTuCustAccountDepositPrepare"));
            ...
            try
            {
                ...
                if (cs == Object.CustAccountStatus.HesabaParaAktarildDekontBasilacak)
                {
    ...
    CashDepositItem GetCashDepositItem()
    {
        #region CashDepositItem Object

        CashDepositItem cashDepositItem = new CashDepositItem();
        ...
        cashDepositItem.ThirdSender = new Sender()
        {
            SenderId = ValidationHelper.GetGuid(new_3rdSenderId.Value)
        };

        cashDepositItem.CustAccount = new CustAccount()
        {
            CustAccountId = ValidationHelper.GetGuid(new_CustAccountId.Value)
        };
        ...
        cashDepositItem.SenderType = ValidationHelper.GetInteger(new_SenderType.Value, 1);

        if (new_SenderType.Value == "3")
        {
            cashDepositItem.SenderPerson = new SenderPerson()
            {
        ...
        cashDepositItem.Calculate = new CalculateDepositRequest();
    ...
    protected void btnFinishOnClickEvent2(object sender, AjaxEventArgs e)
    {
        try
        {
            ICustAccountOperationService<object> _service = new CashDepositService<object>();
            ICustAccountItem cashDepositItem = GetCashDepositItem();
            ...
            if ((string)response.ServiceResponse == Object.CustAccountStatus.HesabaParaAktarildDekontBasilacak)
            {
                Alert(CrmLabel.TranslateMessage("CRM.NEW_CUSTACCOUNTOPERATIONS_MUSTRECIPMENT"));
