...
public partial class CustAccount_Pool_CustAccountTransactionPool : BasePage
{
    ...
    private void PrepareItems()
    {
        ...
        if (!ds.PrvCreate)
        {
            btnDepositOperation.Hide();
            btnCashTransaction.Hide();
    ...
    {
        return "CUSTACCOUNT_TRANSACTION_LIST";
    ...
    protected void GpCustAccountOperationHeaderReload(object sender, AjaxEventArgs e)
    {
        ...
        string strSql = @" SELECT New_CustAccountOperationsId AS ID ,  CustAccountOperationName AS VALUE, 
...
WHERE 1=1 AND tnr.StatusCode=1  AND vncaot.DeletionStateCode=0 AND vncaot .statuscode=1";
        ...
        if (userApproval.CustAccountWithdraw && userApproval.CustAccountDeposit)
        {
            strSql += " AND (vncaot.new_EXTCODE = '002' OR vncaot.new_EXTCODE = '003')";
        }
    ...
    {
        var n = string.Format("Nakit_Yatir_Cek_{0:yyyy_MM_dd_hh_mm_ss}.xls", DateTime.Now);
